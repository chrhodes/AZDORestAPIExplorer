...
{
    public class AZDORestApiExplorerDbContext : DbContext, IAZDORestApiExplorerDbContext
    {
        // TODO(crhodes)
        ...
        public AZDORestApiExplorerDbContext() : base("AZDORestApiExplorer_DB")
        {
            Int64 startTicks = Log.CONSTRUCTOR("Enter", Common.LOG_CATEGORY);
            //Database.SetInitializer(new MigrateDatabaseToLatestVersion<AZDORestApiExplorerDbContext, Configuration>());

            // There are four database initialization strategies

            // 1. CreateDatabaseIfNotExists (default)

            // Database.SetInitializer<AZDORestApiExplorerDbContext>(new CreateDatabaseIfNotExists<AZDORestApiExplorerDbContext>());

            // 2. DropCreateDatabaseIfModelChanges

            // Database.SetInitializer<AZDORestApiExplorerDbContext>(new DropCreateDatabaseIfModelChanges<AZDORestApiExplorerDbContext>());

            Database.SetInitializer<AZDORestApiExplorerDbContext>(
                new DropCreateDatabaseIfModelChanges<AZDORestApiExplorerDbContext>());

            // 3. DropCreateDatabaseAlways

            // Database.SetInitializer<AZDORestApiExplorerDbContext>(new DropCreateDatabaseAlways<AZDORestApiExplorerDbContext>());

            // 4. Custom DB Initializer

            //Database.SetInitializer<AZDORestApiExplorerDbContext>(new AZDORestApiExplorerDbContextDatabaseInitializer());
        ...
        public override int SaveChanges()
        {
            ...
            try
            {
                ...
                Log.PERSISTENCE($"(AZDORestApiExplorerDbContext) Exit ({result})", Common.LOG_CATEGORY, startTicks);
        ...
        {
            Int64 startTicks = Log.PERSISTENCE("(AZDORestApiExplorerDbContext) Enter", Common.LOG_CATEGORY);
